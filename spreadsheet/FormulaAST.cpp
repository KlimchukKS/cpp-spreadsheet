...
// +(A / B) - always okay (the resulting binary op has the highest grammatic precedence)
    ...
    public:
        ...
        virtual double Evaluate(std::function<double(Position)> arg) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<double(Position)> arg) const override {
                switch (type_) {
                    case Add:
                        if (!std::isfinite(lhs_->Evaluate(arg) + rhs_->Evaluate(arg))) {
                            throw FormulaError(FormulaError::Category::Div0);
                        }

                        return lhs_->Evaluate(arg) + rhs_->Evaluate(arg);
                    case Subtract:
                        if (!std::isfinite(lhs_->Evaluate(arg) - rhs_->Evaluate(arg))) {
                            throw FormulaError(FormulaError::Category::Div0);
                        }
                ...
                throw FormulaException("No correct operator");
            }
...
double FormulaAST::Execute(std::function<double(Position)> arg) const {
